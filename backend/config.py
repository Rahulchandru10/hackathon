...
class Settings(BaseSettings):
    ...
    # Explicitly overridden to False via your active .env file for cloud routing
    ...
    # ─── ACTIVE NEO4J INFRASTRUCTURE CONFIGURATION ───────────────────────────
    # Pydantic will match your .env values on boot and overwrite these defaults
    NEO4J_URI: str = Field(default="bolt://localhost:7687")
    NEO4J_USER: str = Field(default="neo4j")
    NEO4J_PASSWORD: str = Field(default="sentinelneo4j")
    
    # ─── VECTOR DB & CACHING LAYERS ──────────────────────────────────────────
    ...
    def get_db_url(self) -> str:
        """Return the correct SQL DB URL based on LOCAL_MODE configuration."""
        if self.LOCAL_MODE:
            return "sqlite+aiosqlite:///./sentinel_local.db"
        return self.POSTGRES_URL

    class Config:
        # Dynamically targets and binds configuration keys from the workspace root
        env_file = ".env"
        env_file_encoding = "utf-8"
        
        # 'allow' protects your custom variables from being dropped during initialization handshakes
        extra = "allow" 

# Instant instantiation for system-wide imports
settings = Settings()
